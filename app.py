...
YOLOv11 · Pipeline Detection · water_leak_best.pt
...
def is_fault(name: str) -> bool:
    nl = name.lower()
    return not ("legit" in nl or "pipeline" in nl and "legit" in nl)

# ───────────────────────────────────────────────────────────────
# MODEL LOADER
# ───────────────────────────────────────────────────────────────
MODEL_PATH = "water_leak_best.pt"
...
<div class="card">
  ...
  <div class="sys-row">
    ...
    <span class="text-mono" style="font-size:11px;color:var(--text2)">water_leak_best.pt</span>
  </div>
  <div class="sys-row">
...
with tab_settings:
    ...
    with col_s1:
        ...
        if not class_names:
            st.markdown(
                '<div style="font-size:12px;color:var(--text3)">No class names loaded — check water_leak_best.pt</div>',
                unsafe_allow_html=True,
            )
        st.markdown("</div>", unsafe_allow_html=True)
