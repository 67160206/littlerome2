...
def is_fault(name: str) -> bool:
    nl = name.lower()
    # pipeline_legit = OK, everything else = FAULT
...
# Also try path relative to this script file (for Streamlit Cloud)
_script_dir = os.path.dirname(os.path.abspath(__file__))
_model_candidates = [
    MODEL_PATH,
    os.path.join(_script_dir, "best.pt"),
    "/mount/src/littlerome2/best.pt",
    "/mount/src/littlerome1.0/best.pt",
]
...
model_ok = False
model = None
class_names = ["pipeline_legit", "water_leak", "pipe_crack", "flame", "corrosion"]
_model_error = ""

for _candidate in _model_candidates:
    try:
        if os.path.exists(_candidate):
            model = load_model(_candidate)
            model_ok = True
            class_names = list(model.names.values()) if model.names else class_names
            break
    except Exception as e:
        _model_error = str(e)

if not model_ok:
    st.error(f"❌ ไม่พบไฟล์ best.pt — ตรวจสอบว่า push ขึ้น GitHub แล้ว\n\nError: {_model_error or 'File not found'}")

# ───────────────────────────────────────────────────────────────
# INFERENCE HELPER
# ───────────────────────────────────────────────────────────────
def run_inference(img_pil: Image.Image, conf: float = 0.5):
    """
    Run YOLOv11 inference on a PIL image.
    ...
    results = model(img_np, conf=conf, verbose=False)[0]
...
with tab_settings:
    ...
    with col_s1:
        ...
        st.markdown(
            f"""
<div class="card-row"><span class="text-muted">Model File</span><span class="text-mono">{MODEL_PATH}</span></div>
<div class="card-row"><span class="text-muted">Classes</span><span class="text-mono">{len(class_names)}</span></div>
<div class="card-row" style="border:none"><span class="text-muted">Framework</span><span class="text-mono">Ultralytics YOLOv11</span></div>
""",
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
